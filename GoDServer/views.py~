...
def getGameList(request):
    ...
    if bool(Waiting_Games):
        with open('waiting_games.pkl', 'rb') as input:
            Waiting_Games = pickle.load(input)
...
def getGameState(request):
    ...
    if bool(Waiting_Games):
        with open('waiting_games.pkl', 'rb') as input:
            Waiting_Games = pickle.load(input)
    if bool(Playing_Games):
        with open('playing_games.pkl', 'rb') as input:
            Playing_Games = pickle.load(input)
    #xyz = Waiting_Games
...
def createGame(request):
    ...
    with open('waiting_games.pkl', 'wb') as output:
        pickle.dump(Waiting_Games, output, pickle.HIGHEST_PROTOCOL)
    response_data = gameDictionary(game)
    return JsonResponse(response_data)
...
def makeTurn(request):
    ...
    with open('playing_games.pkl', 'wb') as output:
        pickle.dump(Playing_Games, output, pickle.HIGHEST_PROTOCOL)
    return JsonResponse(response_data)
